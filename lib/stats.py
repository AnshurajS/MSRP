...
def compute_dataset_statistics(datafiles, output_dir, which=None):
    """
    ...
    datafiles : list of str
        List of .npz files containing the data
    output_dir : str
        Directory to save statistics files
    which : str, optional
        'trans', 'emit', or None for full data
    """
    all_x = []
    all_y = []
    ...
    y_min = np.min(all_y, axis=0)
    y_max = np.max(all_y, axis=0)
    
    # Save statistics
    suffix = f"_{which}" if which else ""
    
    np.save(os.path.join(output_dir, f'fmean{suffix}.npy'), 
            np.concatenate([x_mean, y_mean]))
    np.save(os.path.join(output_dir, f'fstdev{suffix}.npy'), 
            np.concatenate([x_std, y_std]))
    np.save(os.path.join(output_dir, f'fmin{suffix}.npy'), 
            np.concatenate([x_min, y_min]))
    np.save(os.path.join(output_dir, f'fmax{suffix}.npy'), 
            np.concatenate([x_max, y_max]))
    
    print(f"Statistics computed and saved to {output_dir}")
...
def get_stats(datafiles, ilog, olog, ishape, oshape, statsaxes, perc=25, 
    ...
    for i in range(len(datafiles)):
        ...
        x = data['x']
        if x.ndim ==2 and x.shape[1] > ishape[-1]:
            x = x[:, :ishape[-1]]
        if x.shape[1:] == ishape:
            pass
        elif x.shape == ishape:
        ...
        y = data['y']
        if y.ndim ==2 and y.shape[1] != oshape[-1]:
            y = y[:, :oshape[-1]]
        if y.shape[1:] == oshape:
            pass
        elif y.shape == oshape:
