...
import sys, os
os.environ['TF_DETERMISNISTIC_OPS'] = '1'
...
import MARGE.lib.loader as L
from MARGE.lib.loader import load_activation
    ...
    if activations is not None and act_params is not None:
        ...
        if len(activations) != len(act_params):
            ...
            for j in range(len(activations)):
                activations[j] = load_activation(activations[j],
                                                   act_params[j])
...
def load_TFdataset(files, ncores, batch_size, buffer_size,
                   ...
                   x_mean=None, x_std=None, y_mean=None, y_std=None,
                   x_min=None,  x_max=None, y_min=None,  y_max=None,
                   scalelims=None, shuffle=False, which='emit'):
    """
    Builds data loading pipeline for TFRecords, then optionally slices the
    outputs in half (first oshape//2 = transmission, last oshape//2 = emission).
    """
